...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Urbexs is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Urbexs is downloading blocks...");
...
Value getwork(const Array &params, bool fHelp) {
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Urbexs is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Urbexs is downloading blocks...");
