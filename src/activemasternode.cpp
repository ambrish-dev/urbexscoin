...
//
// Bootup the masternode, look for a 3000 ICinput and register on the network
//
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if (status != MASTERNODE_REMOTELY_ENABLED)
        {
            ...
            if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
                ...
                /* rewards are not supported in Urbexs.conf */
...
{
    CUrbexcoinAddress address(collateralAddress);
