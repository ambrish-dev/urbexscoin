...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeUrbexsAmount"))
        settings.setValue("nAnonymizeUrbexsAmount", 1000);
    nAnonymizeUrbexsAmount = settings.value("nAnonymizeUrbexsAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeUrbexsAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeUrbexsAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeUrbexsAmount:
            return QVariant(nAnonymizeUrbexsAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeUrbexsAmount:
            nAnonymizeUrbexsAmount = value.toInt();
            settings.setValue("nAnonymizeUrbexsAmount", nAnonymizeUrbexsAmount);
            emit AnonymizeUrbexsAmountChanged(nAnonymizeUrbexsAmount);
            break;
        default:
            break;
        }
