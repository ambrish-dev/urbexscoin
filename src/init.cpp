...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Urbexs.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Urbexsd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeurbexsamount=<n> " + _("Keep N Urbexs anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Urbexs is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Urbexs is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Urbexs version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Urbexs server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Urbexs") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Urbexs to complete") << "\n";
    ...
    nAnonymizeUrbexsAmount = GetArg("-anonymizeurbexsamount", 0);
    if(nAnonymizeUrbexsAmount > 999999) nAnonymizeUrbexsAmount = 999999;
    if(nAnonymizeUrbexsAmount < 2) nAnonymizeUrbexsAmount = 2;
    ...
    LogPrintf("Anonymize Urbexs Amount %d\n", nAnonymizeUrbexsAmount);
