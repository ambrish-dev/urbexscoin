...
SOURCES_PATH="$SOURCES_PATH_DEFAULT/urbexscoin"
BACKUP_PATH_DEFAULT=$HOME/urbexsbackup
BACKUP_PATH=$BACKUP_PATH_DEFAULT
REPO="https://github.com/urbexscoin/urbexscoin"
BRANCH="master"
SWAP_FILE="$HOME/urbexscoin-swap"
...
if [ -d "../.git" ]; then
	...
	if [ -f "$SOURCES_PATH/bin/urbexsd" ]; then
		rm -f $SOURCES_PATH/bin/urbexsd
		echo -e "${BLUE}Removed old binary in $SOURCES_PATH/bin/${NC}"
	fi
	if [ -f "../bin/urbexsd" ]; then
      rm ../bin/urbexsd
	...
	SOURCES_PATH="${SOURCES_PATH:-$SOURCES_PATH_DEFAULT}/urbexscoin"
...
if [ -f "/usr/local/bin/urbexsd" ]; then
  sudo rm -f /usr/local/bin/urbexsd
  echo -e "${BLUE}Removed urbexsd in /usr/local/bin/ - To be replaced with new version${NC}"
fi
...
if [ ! -f $SOURCES_PATH/bin/urbexsd ]; then
	echo -e "\n${RED}ERROR: urbexsd binary could not be created${NC}"
	exit -2
fi
...
sudo cp $SOURCES_PATH/bin/urbexsd /usr/local/bin/
sudo strip /usr/local/bin/urbexsd

# Backup data dir
if [ -d "$HOME/.Urbexs/" ]; then
	echo -e "\n${GREEN}Backing up ~/Urbexs (including wallet.dat)${NC}"
...
# Done
echo -e "\n${GREEN}Upgrade Complete - You can now run urbexsd or fill out the config file in ~/Urbexs${NC}"
